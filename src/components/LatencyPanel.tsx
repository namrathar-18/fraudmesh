...
export function LatencyPanel({ latency, stats }: { latency: LatencySample[]; stats: PipelineStats }) {
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle = 'rgba(240, 71, 95,0.7)'
    ctx.setLineDash([5, 4])
    ctx.beginPath(); ctx.moveTo(pad.l, y(100)); ctx.lineTo(w - pad.r, y(100)); ctx.stroke()
    ctx.setLineDash([])
    ctx.fillStyle = 'rgba(240, 71, 95,0.9)'
    ctx.fillText('SLO 100', w - pad.r - 48, y(100) - 4)

    if (latency.length > 1) {
      // area
      const grad = ctx.createLinearGradient(0, pad.t, 0, h)
      grad.addColorStop(0, 'rgba(245, 179, 1,0.28)')
      grad.addColorStop(1, 'rgba(245, 179, 1,0)')
      ctx.beginPath()
    ...
    <div className="panel">
      <div className="panel-head">
        ...
        <div className="pill-row">
          ...
          <span className="pill" style={{ color: stats.p99 < 100 ? 'var(--green)' : 'var(--red)', borderColor: stats.p99 < 100 ? 'rgba(33,192,122,0.4)' : 'rgba(240, 71, 95,0.4)' }}>
